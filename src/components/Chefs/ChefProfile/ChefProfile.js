...
export default function ChefProfile() {
  ...
  const handleImg = (e) => {
    if (e?.target?.files[0]) {
      ...
      const myurl = `${process.env.REACT_APP_base_url}api/chef/upload-img`;
  ...
  const submitHandler = async (e) => {
    ...
    if (nameValidate()) {
      if (isPicUpload) {
        ...
        const myurl = `${process.env.REACT_APP_base_url}api/chef/update-profile`;
    ...
    if (passValidate()) {
      const myurl = `${process.env.REACT_APP_base_url}api/chef/change-password`;
      var bodyFormData = new URLSearchParams();
      bodyFormData.append("auth_code", process.env.REACT_APP_AUTH_CODE);
                    ...
                    <div className="rk-edit-profile-head">
                      <div className="rk-edit-profile-img">
                        <img
                          src={
                            isPicUpload
                              ? `${process.env.REACT_APP_base_url}uploads/chef/${pictureName}`
                              : data.image
                          }
                          alt="profile-img"
                          className="img-fluid"
